<script lang="ts">
  ...
  const sanctionsData = [
    {
      ...
      reasonKey: 'insultsVoice',
      statusKey: 'active',    statusStyle: 'bg-emerald-100 text-emerald-700 border-emerald-200',
    },
    {
      type: 'TIMEOUT', typeStyle: 'text-amber-600 bg-amber-50 border-amber-200',
      target: 'Maxou',      mod: 'Zenox', dur: '1h',
      reasonKey: 'spamGeneral',
      statusKey: 'expired',   statusStyle: 'bg-gray-100 text-gray-500 border-gray-200',
    },
    {
      type: 'WARN',    typeStyle: 'text-orange-500 bg-orange-50 border-orange-200',
      target: 'Kylian',     mod: 'Lena',  dur: '',
      reasonKey: 'unauthorizedAd',
      statusKey: 'active',    statusStyle: 'bg-emerald-100 text-emerald-700 border-emerald-200',
    },
    {
      type: 'KICK',    typeStyle: 'text-violet-600 bg-violet-50 border-violet-200',
      target: 'xX_hacker',  mod: 'Aiden', dur: '',
      reasonKey: 'raidAttempt',
      statusKey: 'resolved',   statusStyle: 'bg-blue-100 text-blue-600 border-blue-200',
    },
    {
      type: 'BAN',     typeStyle: 'text-rose-600 bg-rose-50 border-rose-200',
  ...
  const TEXT = {
    fr: {
      title: 'Sanctions',
      subtitle: 'Historique des sanctions et rapports',
      newSanctionBtn: '+ Sanctionner',
      searchPlaceholder: 'Rechercher cible, raison, type…',
      filters: ['Type', 'Statut'],
      columns: { type: 'Type', target: 'Cible', moderator: 'Modérateur', reason: 'Raison', status: 'Statut', report: 'Rapport' },
      view: 'Voir',
      noResults: 'Aucune sanction trouvée',
      reasons: {
        insultsVoice: 'Insultes répétées en vocal',
        spamGeneral: 'Spam dans #général',
        unauthorizedAd: 'Publicité non autorisée',
        raidAttempt: 'Tentative de raid',
        spamBot: 'Bot de spam automatique',
      },
      statusLabels: { active: 'ACTIF', expired: 'EXPIRÉ', resolved: 'RÉSOLU' },
      config: {
        escalationTitle: 'Échelles progressives',
        tables: [
          { name: 'Spam', tiers: ['WARN', 'TIMEOUT 1h', 'BAN'] },
          { name: 'Toxicité', tiers: ['WARN', 'TIMEOUT 24h', 'KICK', 'BAN'] },
        ],
        globalOptionsTitle: 'Options globales',
        options: [
    ...
    en: {
      title: 'Sanctions',
      subtitle: 'History of sanctions and reports',
      newSanctionBtn: '+ New Sanction',
      searchPlaceholder: 'Search target, reason, type…',
      filters: ['Type', 'Status'],
      columns: { type: 'Type', target: 'Target', moderator: 'Moderator', reason: 'Reason', status: 'Status', report: 'Report' },
      view: 'View',
      noResults: 'No sanctions found',
      reasons: {
        insultsVoice: 'Repeated insults in voice chat',
        spamGeneral: 'Spam in #general',
        unauthorizedAd: 'Unauthorized advertising',
        raidAttempt: 'Raid attempt',
        spamBot: 'Automated spam bot',
      },
      statusLabels: { active: 'ACTIVE', expired: 'EXPIRED', resolved: 'RESOLVED' },
      config: {
        escalationTitle: 'Escalation Ladders',
        tables: [
          { name: 'Spam', tiers: ['WARN', 'TIMEOUT 1h', 'BAN'] },
          { name: 'Toxicity', tiers: ['WARN', 'TIMEOUT 24h', 'KICK', 'BAN'] },
        ],
        globalOptionsTitle: 'Global Settings',
        options: [
          ['Cross-server propagation', true],
          ['Automatic report generation', false],
          ['Notify target by DM', true],
        ],
      },
    },
  };
  ...
  const sanctions = $derived(sanctionsData.map(s => ({
    ...s,
    reason: t.reasons[s.reasonKey as keyof typeof t.reasons],
    status: t.statusLabels[s.statusKey as keyof typeof t.statusLabels],
  })));
...
<div class="flex h-full flex-col font-body text-on-surface overflow-hidden">
  ...
  <div class="flex items-center justify-between border-b border-outline-variant/10 bg-linear-to-r from-surface-container/80 to-surface-container-low/50 px-5 py-3.5 shrink-0">
    <div class="flex items-center gap-3">
      ...
      <div>
        <p class="text-sm font-black leading-tight">{t.title}</p>
        <p class="text-[9px] text-on-surface-variant/60">{t.subtitle}</p>
      </div>
    </div>
  ...
  {#if activeTab === 'Sanctions'}
    ...
    <div class="grid grid-cols-[0.6fr_0.9fr_0.8fr_1.5fr_0.7fr_0.5fr] gap-x-3 border-b border-outline-variant/10 bg-surface-container/50 px-5 py-2.5 text-[8px] font-black uppercase tracking-widest text-on-surface-variant/45 shrink-0">
      ...
      <span>{t.columns.moderator}</span>
      <span>{t.columns.reason}</span>
      <span>{t.columns.status}</span>
      <span>{t.columns.report}</span>
    </div>
    ...
    <div class="flex-1 divide-y divide-outline-variant/8 overflow-y-auto">
      {#each filtered as s}
        <div class="group grid grid-cols-[0.6fr_0.9fr_0.8fr_1.5fr_0.7fr_0.5fr] gap-x-3 items-center px-5 py-3.5 transition-colors hover:bg-surface-container-low/40">
          ...
          <span class="text-[10px] font-black text-primary cursor-pointer hover:underline">{t.view}</span>
        </div>
      {/each}
      {#if filtered.length === 0}
        <div class="py-12 text-center text-[10px] font-bold text-on-surface-variant/40">{t.noResults}</div>
      {/if}
    </div>
  ...
  {:else}
    ...
    <div class="flex-1 overflow-y-auto p-5 space-y-4">
      <div class="rounded-xl border border-outline-variant/20 bg-surface-container-lowest p-4">
        <p class="text-[11px] font-black mb-3">{t.config.escalationTitle}</p>
        <div class="space-y-3">
          {#each t.config.tables as table}
      ...
      <div class="rounded-xl border border-outline-variant/20 bg-surface-container-lowest p-4 space-y-3">
        <p class="text-[11px] font-black mb-3">{t.config.globalOptionsTitle}</p>
        {#each t.config.options as [label, on]}
